-- =============================================================================
-- Dalgona Game - Configuration
-- Fantasy Theme Single Player Edition
-- =============================================================================

Config = {}
Config.Locale = GetConvar('dalgona_locale', 'it') -- Language: 'en' or 'it'
Config.GameName = "Runa Forging"

Config.MinimumParticipants = 1 -- Single player mode
Config.Fee = 0 -- No fee for single player mode
...
Config.GameStartInterval = 10000 -- milliseconds

-- NPCs Configuration (disabled for single player)
Config.EnableNPCs = {
    EnableGuards = false,
    EnableParticipants = false,
}
...
Config.GameAreaRadius = 50.0 -- Maximum distance from game area to use crafting table

Config.ShowWinnerMessageGlobally = false
Config.InGameTick = function(playerPed)
    -- Disable controls during game
    DisableControlAction(2, 37, true) -- Weaponwheel
    DisablePlayerFiring(playerPed, true) -- Firing
    DisableControlAction(0, 45, true) -- Reloading
    DisableControlAction(0, 24, true) -- Attacking
...
Config.SpawnCoords = {
    GameStarted = {
        vec3(2081.37, 3343.61, 46.86), -- Near crafting table in desert
        vec3(2081.370117, 3340.610107, 45.828491), -- Crafting table exact coordinates
        vec3(2081.370117, 3340.610107, 45.881866), -- Rock spawn exact coordinates
...
-- Rock Prop Configuration for Single Player Mode
Config.RockProp = {
    Enabled = true,
    ModelHash = -1215378248, -- prop_rock_4_c
    InteractionDistance = 3.0, -- Distance to interact
    SpawnAtPlayerStart = false, -- Spawn at fixed location near table
    OffsetFromPlayer = vec3(0.0, 2.0, 0.0), -- Offset from player position
    BlipEnabled = true,
    BlipSprite = 486,
    BlipColor = 3,
    BlipScale = 0.7,
    BlipName = "Dalgona Stone",
}

-- Crafting Table Configuration for Rune Crafting
Config.CraftingTable = {
    Enabled = true,
    ModelHash = `prop_planer_01`, -- Tavolo da lavoro rigido
    Position = vec3(2081.37, 3343.61, 46.86), -- Posizione principale
    Rotation = vec3(0.0, 0.0, 90.0), -- Rotazione del tavolo
    InteractionDistance = 2.5, -- Distanza di interazione aumentata
    BlipEnabled = true,
    BlipSprite = 351, -- Blip da lavoro/crafting
    BlipColor = 5, -- Colore giallo
    BlipScale = 0.8,
    BlipName = "Runa Forging",
}

-- Fixed positions to avoid overlap
Config.FixedPositions = {
    CraftingTable = vec3(2081.37, 3343.61, 46.86), -- Tavolo da lavoro
    Rock = vec3(2081.37, 3340.61, 46.86), -- Roccia 3 metri indietro rispetto al tavolo
}

-- Minigame complexity (adjust for difficulty)
-- Higher values = easier, Lower values = harder
Config.MinigameComplexityCheck = 0.01  -- Further increased precision for more skill-based gameplay

-- Experimental settings
Config.FeeItem = '' -- Required item to play
-- Config.RewardItem = 'diamond' -- Reward item on success (commented out to use rune system instead)
